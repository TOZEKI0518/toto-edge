...
def resolve_pair_candidates(
    ...
    home_key = clean_text(fixture_home)
    away_key = clean_text(fixture_away)
    return prediction_lookup.get((home_key, away_key), [])


def infer_round_target_date(
    round_fixtures: pd.DataFrame,
    predictions: pd.DataFrame,
    prediction_lookup: dict[tuple[str, str], list[int]],
    window_days: int,
) -> pd.Timestamp | None:
    """Infer the historical date shared by most fixture-pair candidates."""
    candidate_dates: list[pd.Timestamp] = []

    fixture_candidate_indices: list[list[int]] = []
    for fixture in round_fixtures.itertuples(index=False):
        indices = resolve_pair_candidates(
            fixture.home_team,
            fixture.away_team,
            predictions,
            prediction_lookup,
        )
        fixture_candidate_indices.append(indices)
        candidate_dates.extend(
            pd.Timestamp(value).normalize()
            for value in predictions.loc[indices, "match_date"].tolist()
            if pd.notna(value)
        )
    ...
    for target_date in unique_dates:
        ...
        for indices in fixture_candidate_indices:
            if not indices:
                continue

            distances = (
                predictions.loc[indices, "match_date"].dt.normalize()
                - target_date
            ).abs().dt.days
            minimum = int(distances.min())

            if minimum <= window_days:
                matched_fixture_count += 1
                total_distance += minimum

        # Maximise covered fixtures, then minimise distance, then prefer latest.
        score = (
            matched_fixture_count,
            -total_distance,
            int(target_date.value // 10**9),
        )
        if score > best_score:
            best_score = score
...
def match_predictions_to_fixtures(
    ...
    *,
    ...
    """Match by normalized pair and a date inferred once per toto round."""
    prediction_lookup: dict[tuple[str, str], list[int]] = {}
    ...
    for round_no, round_fixtures in fixtures.groupby("round_no", sort=True):
        ...
        target_date = infer_round_target_date(
                    ...
                    unmatched_rows.append(
                        {
                            ...
                            "home_team": fixture.home_team,
                            "away_team": fixture.away_team,
                            "reason": "outside_round_date_cluster",
                            "date_distance_days": date_distance,
                            "inferred_round_date": target_date.strftime(
                                "%Y-%m-%d"
                            ),
        ...
        coverage_rows.append(
            {
                ...
                "coverage_rate": (
                    round_matched / fixture_count
                    if fixture_count
                    else 0.0
                ),
                "no_team_match_count": no_team_match,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--date-window-days", type=int, default=18)
    parser.add_argument(
...
def main() -> None:
    ...
    print(
        "Mean round coverage        : "
        f"{coverage['coverage_rate'].mean():.4f}"
    )
