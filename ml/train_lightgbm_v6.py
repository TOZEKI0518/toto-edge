...
LOGGER = logging.getLogger("lightgbm_v6")
...
class BacktestConfig:
    """LightGBM v6 rolling-backtest configuration."""
...
class EvaluationMetrics:
    """Overall or per-window multiclass metrics."""
...
def load_dataset(path: Path) -> pd.DataFrame:
    ...
    required = {"match_card_id", "match_date", TARGET_COLUMN}
    ...
    if duplicate_ids:
        raise ValueError(
            f"Training dataset contains {duplicate_ids} duplicate IDs."
        )
...
def feature_columns(frame: pd.DataFrame) -> list[str]:
    ...
    if not h2h_features:
        raise ValueError(
            "No H2H features found. Confirm training_dataset_v3.csv."
        )
...
def build_model(config: BacktestConfig) -> Pipeline:
    """Build LightGBM with the same parameters as fixed v5."""
    return Pipeline(
        [
...
def align_probabilities(
    ...
    for source_index, label in enumerate(model_classes):
        if label in CLASS_ORDER:
            aligned[:, CLASS_ORDER.index(label)] = (
...
def rolling_backtest(
    ...
    while test_start <= last_date:
        ...
        if not train.empty and not test.empty:
            ...
            raw_probabilities = model.predict_proba(test[features])
            model_classes = list(
            ...
            normalized = (
                importance_values / total_importance
    ...
    if not prediction_parts:
        raise RuntimeError("No backtest windows were produced.")
...
def save_outputs(
    ...
    predictions.to_csv(
        config.output_dir / "lgbm_v6_predictions.csv",
        index=False,
        encoding="utf-8-sig",
    )
    windows.to_csv(
        config.output_dir / "lgbm_v6_windows.csv",
        index=False,
        encoding="utf-8-sig",
    )
    importance_summary.to_csv(
        config.output_dir / "lgbm_v6_feature_importance.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    pd.DataFrame([summary]).to_csv(
        config.output_dir / "lgbm_v6_summary.csv",
        index=False,
        encoding="utf-8-sig",
    )
    (
        config.output_dir / "lgbm_v6_summary.json"
    ).write_text(
        json.dumps(
            summary,
            ensure_ascii=False,
...
def train_final_model(
    ...
    model_path = config.output_dir / "lightgbm_v6.joblib"
    joblib.dump(
        {
            "model": model,
            "features": features,
...
def print_results(
    ...
    print("=" * 72)
    print("Project Alpha LightGBM v6")
    print("=" * 72)
    print(
        f"Dataset              : "
