...
LOGGER = logging.getLogger("lightgbm_v5")
CLASS_ORDER = ["A", "D", "H"]
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Project Alpha LightGBM v5 date-based backtest."
    )
    parser.add_argument(
        "--input",
        type=Path,
...
def main() -> None:
    ...
    joblib.dump(
        ...
        config.output_dir / "lightgbm_v5.joblib",
    )

    print_results(
        "Project Alpha LightGBM v5",
        frame,
        features,
        predictions,
        overall,
